            ...
            for (int x=0; x<width; x++) {
                for (int ch=0; ch<BytesPerPixel; ch++) {
                    if (round(Imagedata[y][x][ch])==lv){
                        ...
                        else if (lv>=Quartile[ch][1] && lv<=Quartile[ch][2] && bin[ch][2]<(NumberOfPixels/4)) {
                            ImageQuantize[y][x][ch]=2;
                            sum[ch][2]+=lv;
                            bin[ch][2]++;
                        }
                        else if (lv<=Quartile[ch][0] && bin[ch][0]<(NumberOfPixels/4) ) {
                            ImageQuantize[y][x][ch]=0;
                            sum[ch][0]+=lv;
                            bin[ch][0]++;
                        }
                        else if (lv>=Quartile[ch][2] && bin[ch][3]<(NumberOfPixels/4)) {
        ...
        for (int y=0; y<height; y++) {
            for (int x=0; x<width; x++) {
                for (int lv=0; lv<4; lv++) {
                    if (ImageQuantize[y][x][ch] == lv) {
                        ImageQuantize[y][x][ch]=(unsigned char)NewRGBLv[ch][lv];
                    }
                }
            }
        }
