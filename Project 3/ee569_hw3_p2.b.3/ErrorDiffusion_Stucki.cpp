//
//  applyErrorDiffusion_Stucki.cpp
//  HW3_P2.b.3
//
//  Apply "Stucki" error diffusion method on a raw square image (512x512 is defaulted according to the homework assignment)
//  Using 5x5 filter
//
...
#include <math.h>
#include <stdlib.h>
#ifndef ERR_DIFF_STUCKI
#define ERR_DIFF_STUCKI
...
int main(int argc, const char * argv[])
{
    ...
    // argv[2] = "/Users/YJLee/Desktop/mandrill_STUCKI.raw" (or i8)
    ...
    for (int y = 0; y < Size; ++y) {
        ...
        if (y%2 == 0)
        {
            for (int x = 0; x < Size; ++x)
            {
                ...
                MetaImage[y+2][x+3][0] += 0.19048*err;
                MetaImage[y+2][x+4][0] += 0.09524*err;
                MetaImage[y+3][x][0]   += 0.04762*err;
                MetaImage[y+3][x+1][0] += 0.09524*err;
                MetaImage[y+3][x+2][0] += 0.19048*err;
                MetaImage[y+3][x+3][0] += 0.09524*err;
                MetaImage[y+3][x+4][0] += 0.04762*err;
                MetaImage[y+4][x][0]   += 0.02381*err;
                MetaImage[y+4][x+1][0] += 0.04762*err;
                MetaImage[y+4][x+2][0] += 0.09524*err;
                MetaImage[y+4][x+3][0] += 0.04762*err;
                MetaImage[y+4][x+4][0] += 0.02381*err;
            }
        }
        ...
        else
        {
            for (int x = (Size-1); x >= 0; --x)
            {
                ...
                MetaImage[y+2][x][0]   += 0.09524*err;
                MetaImage[y+2][x+1][0] += 0.19048*err;
                MetaImage[y+3][x][0]   += 0.04762*err;
                MetaImage[y+3][x+1][0] += 0.09524*err;
                MetaImage[y+3][x+2][0] += 0.19048*err;
                MetaImage[y+3][x+3][0] += 0.09524*err;
