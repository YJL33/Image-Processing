...
//  HW3_P3.b.1
//
//  Apply thinning filter on a horse1 image (360x275 is defaulted according to the homework assignment)
//
//  Steps:
...
unsigned char HitOrMiss1t(unsigned char arr[], int len);       // Pick and mark the candidate from input data
unsigned char HitOrMiss2t(unsigned char arr[], int len);       // Pick and mark the desired pixels from marks

int main(int argc, const char * argv[])
{
    clock_t begin = clock();
    FILE *file;
    int BytesPerPixel=1, Height = 275, Width = 360;                      // Define the variables
        ...
        for (int y=0; y<Height; ++y) {
            for (int x=0; x<Width; ++x) {
                if (ImageOutput[y][x][0] == 1)
                {
                    ...
                    Metadata[y][x][0] = HitOrMiss1t(Window, pixel_count);
                }
                else
                {
                    Metadata[y][x][0] = 0;
        ...
        for (int y=0; y<Height; ++y) {
            for (int x=0; x<Width; ++x) {
                if (Metadata[y][x][0] == 1)
                {
                    ...
                    Metadata2[y][x][0] = HitOrMiss2t(Window, pixel_count);
                    /*
