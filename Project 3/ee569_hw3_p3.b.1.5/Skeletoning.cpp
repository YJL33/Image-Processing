...
unsigned char HitOrMiss1k(unsigned char arr[], int len);       // Pick and mark the candidate from input data
unsigned char HitOrMiss2k(unsigned char arr[], int len);       // Pick and mark the desired pixels from marks
        ...
        for (int y=0; y<Height; ++y) {
            for (int x=0; x<Width; ++x) {
                if (ImageOutput[y][x][0] == 1)
                {
                    ...
                    Metadata[y][x][0] = HitOrMiss1k(Window, pixel_count);
                }
                else
                {
                    Metadata[y][x][0] = 0;
        ...
        for (int y=0; y<Height; ++y) {
            for (int x=0; x<Width; ++x) {
                if (Metadata[y][x][0] == 1)
                {
                    ...
                    Metadata2[y][x][0] = HitOrMiss2k(Window, pixel_count);
                    /*
