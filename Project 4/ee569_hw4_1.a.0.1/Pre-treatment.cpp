...
//  HW4_P1.a.0.1
//
//  Label each connected objects (characters) in training.raw
...
//
//  Steps:
//  0. Read the image (512x256 24-bit)
...
int main(int argc, const char * argv[])
{
    clock_t begin = clock();
    FILE *file;
    int BytesPerPixel=3, Height = 256, Width = 512;               // Define the variables
    int boundary_box_width=224, boundary_box_height=245;          // The size of desired size of boundary box

    //argv[1] = "/Users/YJLee/Desktop/P4_Output/P1/Training.raw"
    //argv[2] = "/Users/YJLee/Desktop/P4_Output/P1/ch/"
    //argv[3] = "/Users/YJLee/Desktop/P4_Output/P1/Labeled.raw"  (if necessary)
    ...
    printf("\nDesired output Character image: %d*%d (pixels), 8-bit white background image\n\n", boundary_box_width, boundary_box_height);
    ...
    int object_count=-1;                           // Count the object
    ...
    for (int y=0; y < Height; ++y)
    {
        for (int x=0; x < Width; ++x)
        {
            if (GreyImage[y][x][0] == 0)                   // This pixel is background
            {
                Label[y][x][0] = 0;                        // Label the background as 0
                LabeledOrNot[y][x][0] = true;
            }
            if (GreyImage[y][x][0] == 255)                 // This pixel is target
            {
                ...
                for (int j = y - 1; j <= y + 1; ++j){      // Get this pixel's neighborhood
                    for (int i = x - 1; i <= x + 1; ++i){
                        if ((j >= 0) && (j < Height) && (i >= 0) && (i < Width)){
                            if ((GreyImage[j][i][0] != 0) && (LabeledOrNot[j][i][0] == true)) {
    ...
    // Combine multiple labels within one object by replacing all labels with the minimum one.
    for (int y=0; y<Height; ++y)
    {
        for (int x=0; x<Width; ++x)
        {
    ...
    for (int i=0; i<NextLabel; ++i) {                                // Re-organize these labels
        if (Linked[i] == i) {                                        // If this label have no root => it's a remaining label, count it.
            object_count++;
            if (object_count != i) {
    ...
    for (int object_id=1; object_id<=object_count; ++object_id) {
        ...
        printf("For character %d, Actual Area = %dx%d, ", object_id, object_width, object_height);
