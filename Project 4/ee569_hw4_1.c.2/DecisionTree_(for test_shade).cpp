//
//  DecisionTree_(for test_shade.raw).cpp (modified from 1.a.2)
//  HW4_P1.c.2
//
//  Use Decision Tree and Minimum distance classifier to classify the unknown image
...
//
//  Min-dist-classifier:
//  step 0.   Read the image and clean some noises
...
//  1. The image size is different, also needs to wipe out some salt noise.
//  2. The unknown characters is now 23
//
//
...
int main(int argc, const char * argv[])
{
    ...
    int NumOfUnknownCharacter = 23;
    int NumberOfFeatures = 12;
    
    // argv[1] = "/Users/YJLee/Desktop/Feat.array"
    // argv[2] = "/Users/YJLee/Desktop/ch/%02d.raw";
    ...
    for (int ch = 0; ch < NumOfUnknownCharacter; ch++) {        // use "for loop" to handle all image iteratively   ***** IMPORTANT: this loop lasts through step 2. *****
        ...
        for (int y=0; y<Height; ++y) {
            for (int x=0; x<Width; ++x) {
                if ((Imagedata[y][x] == 254) && (Imagedata[y-1][x-1] == 0) && (Imagedata[y-1][x] == 0) && (Imagedata[y-1][x+1] == 0) && (Imagedata[y][x-1] == 0) && (Imagedata[y][x+1] == 0) && (Imagedata[y+1][x-1] == 0) && (Imagedata[y+1][x] == 0) && (Imagedata[y+1][x+1] == 0)) {      // Find Salt noise
                    Imagedata[y][x] = 0;
                }
            }
        }
        
        // Step 1. Calculate the features
        ...
        if (Feature[7][ch] == -1) {                // Euler number = -1
            ...
            else
            {
                if (Feature[6][ch] > 0.9) {            // This character is wide
                    ...
                    else if (Feature[8][ch] > 1.5) {     // The character is distributed more in rignt (3)
                        printf("\n==============Guess: 3==============\n");
                    }
                    else
                    {
