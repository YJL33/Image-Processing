...
//  HW4_P1.c.0.2
//
//  Label each connected objects (characters) in Test_shade.raw
...
//
//  Steps:
//  0. Read the image (215x361 24-bit)
...
//  2. Set a threshold when converting the image to grey-level, and also need another threshold to distinguish the background and target.
...
//  4. Dilate the discrete characters
//
...
int main(int argc, const char * argv[])
{
    clock_t begin = clock();
    FILE *file;
    int BytesPerPixel=3, Height = 361, Width = 215;               // Define the variables
    int boundary_box_width=224, boundary_box_height=245;          // The size of desired size of boundary box

    //argv[1] = "/Users/YJLee/Desktop/P4_Output/P1/Test_shade.raw"
    //argv[2] = "/Users/YJLee/Desktop/P4_Output/P1/shade/"
    //argv[3] = "/Users/YJLee/Desktop/P4_Output/P1/shade/Labeled.raw"  (if necessary)
    ...
    for (int y = 0; y < Height; ++y)
    {
        ...
        else
        {
            for (int x = 0; x < Width; ++x)
            {
                if ((x<40) || (x>154) || (y<267) || (y>333)) {
                    GreyImage[y][x][0] = 0;                           // Here we eliminate the undesired boundaries
                }
                else {
                    GreyImage[y][x][0] = 255 - GreyImage[y][x][0];
                }
            }
        }
    }
    ...
    unsigned char background_threshold = 150;
    ...
    for (int y=0; y < Height; ++y)                // Label all pixels
    {
        for (int x=0; x < Width; ++x)
        {
            if (GreyImage[y][x][0] < background_threshold)                  // This pixel is background
            {
                Label[y][x][0] = 0;                        // Label the background as 0
                LabeledOrNot[y][x][0] = true;
            }
            if (GreyImage[y][x][0] >= background_threshold)                  // This pixel is target
            {
                ...
                for (int j = y - 1; j <= y + 1; ++j){      // Get this pixel's neighborhood
                    for (int i = x - 1; i <= x + 1; ++i){
                        if ((j >= 0) && (j < Height) && (i >= 0) && (i < Width)){
                            if ((GreyImage[j][i][0] > background_threshold) && (LabeledOrNot[j][i][0] == true)) {      // This neighbor is target
    ...
    // Special Step: Make the dilation process in discrete characters
    
    unsigned char Label2[Height][Width][1];
    for (int y=0; y<Height; ++y) {
        for (int x=0; x<Width; ++x) {
            if ((x>40) && (x<154) && (y>267) && (y<333)) {
                Label2[y][x][0] =
                ((Label[y][x+4][0]) ||
                 (Label[y+2][x+3][0]) || (Label[y+1][x+3][0]) || (Label[y][x+3][0]) || (Label[y-1][x+3][0]) || (Label[y-2][x+3][0]) ||
                 (Label[y+2][x+2][0]) || (Label[y+1][x+2][0]) || (Label[y][x+2][0]) || (Label[y-1][x+2][0]) || (Label[y-2][x+2][0]) ||
                 (Label[y+2][x+1][0]) || (Label[y+1][x+1][0]) || (Label[y][x+1][0]) || (Label[y-1][x+1][0]) || (Label[y-2][x+1][0]) ||
                 (Label[y+3][x][0]) || (Label[y+2][x][0]) || (Label[y+1][x][0]) || (Label[y][x][0]) || (Label[y-1][x][0]) || (Label[y-2][x][0]) || (Label[y-3][x][0]) ||
                 (Label[y+2][x-1][0]) || (Label[y+1][x-1][0]) || (Label[y][x-1][0]) || (Label[y-1][x-1][0]) || (Label[y-2][x-1][0]) ||
                 (Label[y+2][x-2][0]) || (Label[y+1][x-2][0]) || (Label[y][x-2][0]) || (Label[y-1][x-2][0]) || (Label[y-2][x-2][0]) ||
                 (Label[y+2][x-3][0]) || (Label[y+1][x-3][0]) || (Label[y][x-3][0]) || (Label[y-1][x-3][0]) || (Label[y-2][x-3][0]) ||
                 (Label[y][x-4][0])
    ...
    for (int y=0; y<Height; ++y) {
        for (int x=0; x<Width; ++x) {
            if ((x>40) && (x<154) && (y>267) && (y<333)) {
                Label[y][x][0] = Label2[y][x][0];
            }
        }
    }
    
    for (int y=267; y<333; ++y) {
        for (int x=40; x<154; ++x) {
            if ((x<100) && Label[y][x][0] != 0) {
                Label[y][x][0] = NextLabel;
            }
            if ((x>100) && Label[y][x][0] != 0)
            {
                Label[y][x][0] = NextLabel+1;
            }
        }
    }
    Linked[NextLabel] = NextLabel;
    Linked[NextLabel+1] = NextLabel+1;
    ...
    for (int i=0; i<NextLabel; ++i) {                                // Check all labels
        if (Linked[i] == i) {                                        // If this label have no root => it's a connected object.
            ...
            if ((pixel_count >=20)) {                                // Determine whether this is a character or not: based on actual size (pixel count)
                object_count++;                                      // If size is correct => count it as object (threshold is obtained from observation)
                
                if ((object_count != i) && (pixel_count >=20)) {     // If this object has a wrong number of label
                ...
                else {printf("Remaining Label: %d\n", i);}           // If this character has a correct number of label (no need to re-label)
            }
            else
            {
                for (int y=0; y<Height; ++y) {
